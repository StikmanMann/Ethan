...
export class Ollama extends OllamaBrowser {
    ...
    async createBlob(path) {
        ...
        try {
            await utils.head(this.fetch, `${this.config.host}/api/blobs/${ /* digest */"skbidi"}`); //YEAH IDK
        }
        catch (e) {
            if (e instanceof Error && e.message.includes("404")) {
                ...
                await utils.post(this.fetch, `${this.config.host}/api/blobs/${ /* digest */"skbidi"}`, //YEAH IDK
                readableStream);
            }
            else {
                throw e;
