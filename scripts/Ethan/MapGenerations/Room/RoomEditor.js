...
import { EDirection, pushRoomToWorld, rooms } from "./Rooms";
...
world.beforeEvents.itemUse.subscribe((event) => {
    ...
    if (item.nameTag == roomConnectorSword.nameTag) {
        let posExists = currentRoom.roomConnectors.find((connector) => connector.location.x == block.block.location.x &&
            connector.location.y == block.block.location.y &&
            connector.location.z == block.block.location.z);
        if (posExists) {
            world.sendMessage("Removed Room Connector at §3" + JSON.stringify(posExists));
            currentRoom.roomConnectors = currentRoom.roomConnectors.filter((connector) => connector != posExists);
            return;
        }
        else {
            world.sendMessage("Placed Room Connector at §3" + JSON.stringify(block.block.location));
    ...
    if (item.nameTag == startPositionPickaxe.nameTag) {
        world.sendMessage("Placed Start Position at §3" + JSON.stringify(block.block.location));
        currentRoom.startPosition = block.block.location;
    }
    if (item.nameTag == endPositionAxe.nameTag) {
        world.sendMessage("Placed End Position at §3" + JSON.stringify(block.block.location));
...
const saveRoom = (player) => {
    system.run(() => {
        ...
        showHUD(player, modalForm).then((result) => {
            ...
            const roomName = result.formValues[0];
...
const visualizeRoom = (room) => {
    room.roomConnectors.forEach((connector) => {
        ...
        switch (connector.direction) {
            case EDirection.north:
                DrawFunctions.drawLine(5, VectorFunctions.addVector(connector.location, {
                    x: 0.5,
                    y: 0.5,
                    z: 0.5,
    ...
    DrawFunctions.drawCube(room.startPosition, room.endPosition, "minecraft:basic_flame_particle", 10);
};
...
export const setPlayerToEdit = (player, room) => {
    ...
    player.teleport(VectorFunctions.addVector(currentRoom.startPosition, VectorFunctions.multiplyVector(VectorFunctions.subtractVector(currentRoom.endPosition, currentRoom.startPosition), 0.5)));
