...
const generateRooms = (startingPosition) => {
    ...
    system.run(() => {
        ...
        world.structureManager.place(structures.get(room.index), GlobalVars.overworld, startingPosition);
...
const generateConnectors = (depth) => {
    system.run(() => {
        ...
        for (const connector of copiedConnectors) {
            GlobalVars.overworld.setBlockType(connector.location, "minecraft:glowstone");
            while (true) {
          ...
          I DID NOT FIND A CONNECTOR FOR: ${room.id} and direction: ${connector.direction}}`);
                ...
                world.structureManager.place(structures.get(randomRoom.index), GlobalVars.overworld, VectorFunctions.addVector(VectorFunctions.addVector(connector.location, VectorFunctions.subtractVector(VectorFunctions.smallestOnly(randomRoom.startPosition, randomRoom.endPosition), randomConnector.location)), VectorFunctions.getVectorFromDirection(connector.direction)));
                for (const randomRoomConnector of randomRoom.roomConnectors) {
                    world.sendMessage(VectorFunctions.vectorToString(randomRoomConnector.location) +
                        " == " +
                        VectorFunctions.vectorToString(randomConnector.location));
                    if (VectorFunctions.vectorToString(randomRoomConnector.location) ==
                        VectorFunctions.vectorToString(randomConnector.location)) {
                        continue;
                    }
                    temporaryConnectors.push({
                        direction: randomRoomConnector.direction,
                        location: VectorFunctions.addVector(VectorFunctions.addVector(connector.location, VectorFunctions.getVectorFromDirection(connector.direction)), VectorFunctions.addVector(VectorFunctions.subtractVector(VectorFunctions.smallestOnly(randomRoom.startPosition, randomRoom.endPosition), randomConnector.location), VectorFunctions.subtractVector(randomRoomConnector.location, VectorFunctions.smallestOnly(randomRoom.startPosition, randomRoom.endPosition)))),
                    });
                }
                openConnectors = [];
                break;
        ...
        for (const connector of openConnectors) {
            GlobalVars.overworld.setBlockType(connector.location, "minecraft:sea_lantern");
        }
        if (depth <= 0) {
            return;
        }
...
const generateStructures = async () => {
    system.run(() => {
        for (const room of rooms) {
            ...
            const struct = world.structureManager.createFromWorld(`ethan:${room.id}`, GlobalVars.overworld, room.startPosition, room.endPosition, { saveMode: StructureSaveMode.Memory });
