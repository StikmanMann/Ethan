...
import { octavePerlin } from "../NoiseGenerator/PerlinNoise";
// Block Type Mapping
const getBlock = (num) => {
    if (num < 0.1) {
        return "wool";
        ...
        for (let x = -64 + posX; x < 64 + posX; x++) {
            for (let z = -64 + posZ; z < 64 + posZ; z++) {
                ...
                if (block === "light_gray_wool") {
                    GlobalVars.overworld.fillBlocks(new BlockVolume({ x, y: -50, z }, { x, y: -59, z }), "oak_planks");
                }
                else {
                    GlobalVars.overworld.fillBlocks(new BlockVolume({ x, y: -50, z }, { x, y: -59, z }), "air");
                }
            }
        }
    }
