...
class DrawFunctions {
  ...
  static drawLine(
    ...
    let normalisedVector = VectorFunctions.normalizeVector(
      VectorFunctions.subtractVector(endLocation, startLocation)
    );
    let distance = VectorFunctions.vectorLength(
      VectorFunctions.subtractVector(endLocation, startLocation)
    );
    ...
    for (let i = 0; i < steps; i++) {
      ...
      particleLocation = VectorFunctions.addVector(
        particleLocation,
        VectorFunctions.multiplyVector(normalisedVector, stepSize)
      );
    }
  }
  ...
  static drawSphere(
    ...
    particleName: keyof ParticleList = "minecraft:blue_flame_particle",
    molangVars?
  ) {
    ...
    for (let i = 0; i < steps; i++) {
      let particleLocation = VectorFunctions.addVector(
        centerLocation,
        VectorFunctions.rotateVectorY(addVector, i * stepSize)
      );
  ...
  static drawCube(
    ...
    DrawFunctions.drawLine(
      resolution,
      { x: boxStart.x, y: boxStart.y, z: boxStart.z },
      { x: boxEnd.x, y: boxStart.y, z: boxStart.z },
      particleName,
      molangVars
    );
    DrawFunctions.drawLine(
      resolution,
      { x: boxStart.x, y: boxStart.y, z: boxStart.z },
      { x: boxStart.x, y: boxEnd.y, z: boxStart.z },
      particleName,
      molangVars
    );
    DrawFunctions.drawLine(
    ...
    DrawFunctions.drawLine(
      resolution,
      { x: boxEnd.x, y: boxEnd.y, z: boxEnd.z },
      { x: boxStart.x, y: boxEnd.y, z: boxEnd.z },
      particleName,
      molangVars
    );
    DrawFunctions.drawLine(
      resolution,
      { x: boxEnd.x, y: boxEnd.y, z: boxEnd.z },
      { x: boxEnd.x, y: boxStart.y, z: boxEnd.z },
      particleName,
      molangVars
    );
    DrawFunctions.drawLine(
  ...
  static async drawGraph(steps, startLocation, endLocation, player: Player) {
    return new Promise(async (resolve) => {
      ...
      let distance = VectorFunctions.vectorLengthXZ(
        VectorFunctions.subtractVector(endLocation, startLocation)
      );
      ...
      do {
        ...
        for (let i = xOffset; i < newSteps; i++) {
          ...
          let addVector = VectorFunctions.rotateVectorY(
            VectorFunctions.rotateVectorY(
              { x: stepSize, y: functionY - lastIncrease, z: 0 },
              rotY
            ),
            rotX
          );
      ...
      if (true) {
        ...
        for (let i = xOffset; i < newSteps; i++) {
          ...
          if (
            !CollisionFunctions.insideSphere(
              player.location,
              particleLocation,
              2.25
            )
