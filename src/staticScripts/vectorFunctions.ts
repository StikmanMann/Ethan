...
class VectorFunctions {
   ...
   */
  static addVector(a: Vector3, b: Vector3) {
  ...
  static smallestOnly(a: Vector3, b: Vector3): Vector3 {
    let boxStart = {
      x: a.x < b.x ? a.x : b.x,
      y: a.y < b.y ? a.y : b.y,
      z: a.z < b.z ? a.z : b.z,
   ...
   */
  static multiplyVector(vector: Vector3, scalar: number) {
    return {
      x: vector.x * scalar,
      y: vector.y * scalar,
      z: vector.z * scalar,
   ...
   * @returns {String}
   */
  static vectorToString(vector: Vector3): string {
  ...
  static stringToVector(inputString: string): Vector3 {
    ...
    if (isNaN(x) || isNaN(y) || isNaN(z)) {
      throw new Error(
        "Invalid input: Each component of the vector must be a number."
      );
    }
...
String.prototype.toVector3 = function (): Vector3 {
