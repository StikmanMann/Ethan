...
const playerTrackerNames: Set<string> = new Set();
  ...
  system.run(() => {
    ...
    if (playerTrackerNames.has(item.typeId)) {
      ...
      player.onScreenDisplay.setActionBar(
        `Tracking §l${
          closestPlayer.nameTag
        }§r - Distance: §l${VectorFunctions.vectorLength(
          VectorFunctions.subtractVector(location, closestPlayer.location)
        ).toFixed(1)}m`
      );
    }
  });
...
const getClosestPlayer = (
  originPlayer: Player,
  excludeTeamColor: boolean
): Player | null => {
  const origin = originPlayer.location;
  let closestPlayer: Player = null;
  ...
  for (const player of world.getPlayers()) {
    ...
    let distanceToPlayer = VectorFunctions.vectorLength(
      VectorFunctions.subtractVector(origin, player.location)
    );
