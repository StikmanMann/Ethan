...
world.beforeEvents.playerInteractWithEntity.subscribe((event) => {
  system.run(async () => {
    ...
    shopEntity.teleport(
      VectorFunctions.addVector(event.player.location, { x: 0, y: -3, z: 0 })
    );
...
const checkShopsForBuy = () => {
  for (const shopEntity of activeShops) {
    ...
    const buyer = world.getPlayers({
      name: shopEntity.getDynamicProperty("buyer") as string,
    })[0];
    ...
    const shopEntityInventory = shopEntity.getComponent(
      "minecraft:inventory"
    ).container;
    ...
    for (let i = 0; i < shopEntityInventory.size; i++) {
      ...
      if (buyer.getComponent("cursor_inventory").item) {
        ...
        if (
          item.getLore().length == missingItem.getItem(buyer).getLore().length
        ) {
          buyer.getComponent("cursor_inventory").clear();
          missingItem.buyFunction(
            shopEntity,
            buyer,
            missingItem.getItem(buyer),
            missingItem.getPrice(buyer)
          );
          continue;
        }
      }
      ...
      const buyer_inventory = buyer.getComponent(
        "minecraft:inventory"
      ).container;
