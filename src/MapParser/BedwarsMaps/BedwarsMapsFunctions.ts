...
export const bedwarsStart = async (
  ...
  for (let i = 0; i < bedwarsData.diamondGenerators.length; i++) {
    bedwarsData.diamondGenerators[i] = VectorFunctions.subtractVector(
      VectorFunctions.addVector(bedwarsData.diamondGenerators[i], offset),
      mapData.startLocation
    );
  }
  ...
  for (const team of bedwarsData.teams) {
    ...
    team.bedLocation = VectorFunctions.subtractVector(
...
const bedwarsTick = (mapData: IMapData<EGameMode.BEDWARS>) => () => {
  ...
  for (let i = 0; i < bedwarsData.queuedLapisGenerators.length; i++) {
    ...
    if (queuedLapis.size === 0) { continue; }
    let lapisGenerator = bedwarsData.lapisGenerators[i];
    try {
  ...
  for (let i = 0; i < bedwarsData.queuedDiamondsGenerators.length; i++) {
    ...
    if (queuedDiamonds.size == 0) { continue; }
    let diamondGenerator = bedwarsData.diamondGenerators[i];
    try {
