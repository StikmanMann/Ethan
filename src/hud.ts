...
const tick = () => {
  for (const player of GlobalVars.players) {
    ...
    actionbarMessages.forEach((actionbarMessage, index) => {
      ...
      centerFiller = Math.ceil(
        (maxLength - actionbarMessage.message.length) / 2
      );
...
export const choosePlayer = async (
  showHUDPlayer: Player,
  ignoreSelf: boolean = false,
  playersToChooseFrom = GlobalVars.players
): Promise<Player> => {
    ...
    if (response.selection === 0) {
      ...
      showHUD(showHUDPlayer, searchPlayerPanel).then((res) => {
        ...
        if (filteredPlayers.length === 0) {
          showHUDPlayer.sendMessage(
            `§cNo player found with the name ${res.formValues[0]}\nMake sure to use the name, not the nameTag/nick`
          );
          return;
        }
